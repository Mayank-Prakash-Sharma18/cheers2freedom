...
In client-side rendering, the server takes the data from the database, applies server-side logit to it and then sends the information to the client-side in the form of JSON, XML etc. <br><br>
...
* Create a static website – If you do not intend to add to much functionality in your website, then you can go for a static website (rather than a dynamic website). For example, if you only want a blog where you will only upload articles. But you should not opt for this option if you want to create a much more complex website, say an e-commerce website or a website where you intend to add a lot of customized functionalities. <br><br>
...
In static websites, we do not have a server. The already pages exist in the database of your hosting provider and are sent to the browser as it is. It makes our site safer to cyber-attacks and much faster.  <br><br>
...
{{% alert warning %}}
Note: There are two versions of Wordpress:<br>
...
* Less time consuming: Yes, you can make a static side by using HTML, CSS and JavaScript. But the drawback to this approach is that, you will most probably have to create your own theme first. It’s very time-consuming. While using a static site generator, you can make your site in a jiffy. 
...
Static sites do not suffer from such problems. There is no server and as most of the hacking attempts are aimed as the server, we have already dodged a bullet. You just need to use two-factor authorization in your domain name provider, hosting and github. That’s all. <br><br>
Moreover, as you will learn in the Static site section of this website, our static site is always on our local computer. We work on it and once we are satisfied, we just push all the changes to the github, which in turn updates the hosting provider automatically. Your website is already on your local computer. So, no need to take any backup. 
...
Let us know what you prefer and why. And if you want to get into nitty-gritties of blog creation using Wordpress or HUGO static site generator, do have a look into our free courses under ‘Create a Blog’ section. 
